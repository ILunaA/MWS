...
//For Arduino Mega
//Jump wire A4 (SDA weather shield) to PIN20 (SDA Mega) 
//Jump wire A5 (SCL weather shield) to PIN21 (SCL Mega) 
static const int RXPin = 50, TXPin = 51; //GPS is attached to pin 6 (PWR), pin 5(RX from GPS) and pin 4(RX into GPS)
...
float windspdavg[30] = {
  0.0}; //30 bytes to keep track of 2 minute average
int winddiravg[30] = {
  0}; //30 ints to keep track of 2 minute average
...
void loop()
{
  ...
  if(minutes > 59) {  
    minutes = minutes%60;
    hours ++;
    if(rainHour > 200.0){ //This is a fix for IRQs getting crazy because of heat in the box (Yann: 28 April 2017, reduced to 200 27Sept2017)
